...
interface DashboardData {
  ...
  amounts: {
    total_asset: number; // 總資產金額 (所有 unit_price 加總)
    total_repair: number; // 維修總金額 (所有維修紀錄的 cost 加總)
    total_scrapped: number; // 報廢總金額 (所有已報廢資產的 unit_price 加總)
  };
  ...
  const MiniStat = ({ label, value, icon, color, onClick }: any) => (
    <Paper
      ...
      sx={{
        ...
        bgcolor: (theme) =>
          alpha(color, theme.palette.mode === "dark" ? 0.2 : 0.05),
        border: "1px solid",
        borderColor: (theme) =>
          alpha(color, theme.palette.mode === "dark" ? 0.4 : 0.2),
        borderRadius: 2,
        ...
        >
          <Box sx={{ display: "flex", alignItems: "flex-end", gap: 4 }}>
            ...
            <Stack
              ...
              divider={
                <Box
                  sx={{ width: 1, height: 24, bgcolor: "divider", my: "auto" }}
                />
              }
              sx={{
            ...
            >
              <Box sx={{ minWidth: 100 }}>
                ...
                <Typography
                  ...
                  color="primary.main"
                >
                  {formatCurrency(data.amounts.total_asset)}
                </Typography>
              </Box>
              <Box sx={{ minWidth: 100 }}>
                <Typography
                ...
                <Typography
                  ...
                  color="warning.main"
                >
                  {formatCurrency(data.amounts.total_repair)}
                </Typography>
              </Box>
              <Box sx={{ minWidth: 100 }}>
                <Typography
                ...
                <Typography
                  ...
                  color="text.disabled"
                >
                  {formatCurrency(data.amounts.total_scrapped)}
                </Typography>
              </Box>
            </Stack>
          </Box>
          ...
          <Grid size={{ xs: 12, md: 9 }}>
            <Grid container spacing={1.5}>
              <Grid size={{ xs: 6, sm: 4 }}>
                <MiniStat
                  label="使用中"
                  value={data.stats.in_use}
                  color="#2e7d32"
                  icon={<CheckCircleIcon />}
                  onClick={() => navigate('/inventory?status=使用中')}
                />
              </Grid>
              <Grid size={{ xs: 6, sm: 4 }}>
                <MiniStat
                  label="閒置資產"
                  value={data.stats.idle}
                  color="#00bcd4"
                  icon={<Inventory2Icon />}
                  onClick={() => navigate('/inventory?status=閒置')}
                />
              </Grid>
              <Grid size={{ xs: 6, sm: 4 }}>
                <MiniStat
                  label="借用中"
                  value={data.stats.borrowed}
                  color="#9c27b0"
                  icon={<AssignmentIndIcon />}
                  onClick={() => navigate("/inventory?status=借用中")}
                />
              </Grid>
              <Grid size={{ xs: 6, sm: 4 }}>
                <MiniStat
                  label="維修中"
                  value={data.stats.repair}
                  color="#ed6c02"
                  icon={<BuildIcon />}
              ...
              <Grid size={{ xs: 6, sm: 4 }}>
                <MiniStat
                  ...
                  onClick={() => navigate("/inventory?status=遺失")}
                />
              </Grid>
              <Grid size={{ xs: 6, sm: 4 }}>
                <MiniStat
                  label="已報廢"
                  value={data.stats.scrapped}
                  color="#607d8b"
                  icon={<DeleteForeverIcon />}
                  onClick={() => navigate("/inventory?status=報廢")}
                />
              </Grid>
            </Grid>
          </Grid>
            ...
            <List dense disablePadding>
              {data.todos.map((todo, idx) => (
                <ListItem
                  key={idx}
                  sx={{
                    bgcolor: todo.type === "warning" ? "#fff3e0" : "#ffebee",
                    borderRadius: 1,
                    mb: 1,
                  }}
                >
